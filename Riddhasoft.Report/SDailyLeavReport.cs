...
//using Riddhasoft.HumanResource.Entity;
...
{
    public class SDailyLeavReport
    {
        DB.RiddhaDBContext db;
        public SDailyLeavReport()
        {
        ...
        private void calculateRemaining(AttendanceReportDetailViewModel eachRow, DateTime FromDate)
        {
            ...
            if (!eachRow.NoPunch)
            {
                //var log = db.AttendanceLog.Where(x => x.EmployeeId == eachRow.EmployeeDeviceCode && DbFunctions.TruncateTime(x.DateTime) == DbFunctions.TruncateTime(FromDate) && x.IsDelete == false).ToList();
        ...
        private void calculateLeaveHoliday(AttendanceReportDetailViewModel eachRow, DateTime FromDate)
        {
            ...
            {
                eachRow.OnLeave = "Yes";
                eachRow.LeaveName = leaves.LeaveMaster.Code + "-" + leaves.LeaveMaster.Name;
